...
function dateFormat(date, format = "yyyy-MM-dd HH:mm:ss") {
  ...
  for (let [k, v] of Object.entries(o)) {
    ...
    for (let [index2, value] of Object.entries(lFormat)) {
      if (k == value)
        lFormat[Number(index2)] = "0";
    }
  }
  return lFormat.reverse().join("");
}
async function onion(ctx, ...middleware) {
  let index2 = -1;
  await async function next() {
    if (++index2 >= middleware.length)
      return;
    const auto = middleware[index2].length <= 1;
    await middleware[index2](ctx, next);
    auto && await next();
  }();
}
...
function merge(obj1, obj2, deep = 1, overwrite = false) {
  var _a, _b;
  deep--;
  ...
  for (let [key, val] of entries(obj2)) {
    if (!(obj1[key] instanceof Object) && !overwrite && obj1[key] != void 0 && obj1[key] != null)
      continue;
    if (!(val instanceof Object) || deep <= 0)
      defineProperty(obj1, key, (_a = getOwnPropertyDescriptor(obj2, key)) != null ? _a : {});
    (_b = obj1[key]) != null ? _b : obj1[key] = {};
    if (val instanceof Object && deep > 0)
      merge(obj1[key], val, deep);
  }
  return obj1;
}
var toString = (obj) => Object.prototype.toString.call(obj);
var createEventBus = (map = /* @__PURE__ */ new Map()) => {
  return {
    map,
    on(type, handler) {
    ...
    emit(type, event) {
      const handlers = map.get(type);
      handlers && handlers.forEach((handler) => handler(event != null ? event : type));
    }
  };
};
